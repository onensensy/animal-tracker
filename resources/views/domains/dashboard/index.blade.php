        ...
        <form method="GET">
            <div class="lg:flex lg:space-x-4">
                ...
                <div class="flex-1 mb-4">
                    <div class="flex">
                        <div class="flex-1">
                            <x-select name="trip_id" :options="$trips" value="id" text="name"
                                data-change-submit></x-select>
                        </div>
                ...
                <div class="mb-4 text-center">
                    ...
                    <a href="{{ route('trip.update.alarm-notification', $trip->id) }}"
                        class="btn bg-white mr-2">@icon('bell', 'w-4 h-4 sm:w-6 sm:h-6')</a>
                    <a href="{{ route('trip.update.merge', $trip->id) }}" class="btn bg-white">@icon('git-merge', 'w-4 h-4 sm:w-6 sm:h-6')</a>
                </div>
            </div>
        </form>

        <x-map :trip="$trip" :positions="$positions" :alarms="$alarms" :notifications="$trip_alarm_notifications" :data-map-show-last="$trip->finished()"
            :data-map-positions-url="route('trip.update.position', $trip->id)"></x-map>
