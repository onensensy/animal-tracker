<?php return [
    'device' => 'Move your :count trips to the device',
    'error' => [
        'device-exists' => 'The selected device is no longer available',
        'trips-exists' => 'It is not possible to transfer this device without selecting a destination for your trips',
        'user-exists' => 'The selected user is no longer available',
        'vehicle-exists' => 'The selected Animal is no longer available',
    ],
    'meta-title' => 'Devices > :title > Transfer',
    'save' => 'Transfer',
    'success' => 'The device has been transferred successfully',
    'user' => 'Assign this device to the user',
    'vehicle' => 'Move your :count trips to the Animal',
];
